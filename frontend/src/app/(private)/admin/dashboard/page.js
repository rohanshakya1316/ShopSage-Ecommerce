...
import { BarChart3, Bell, ChevronDown, CircleDollarSign, Eye, LayoutDashboard, LogOut, Package2, Search, Settings, ShoppingCart, TrendingUp, Users } from "lucide-react";
      ...
      try {
        ...
        setRecentOrders(
          orders.slice(0, 4).map((order) => {
            ...
            return {
              id: order.orderNumber || `#ORD-${order._id?.slice(0, 6).toUpperCase() || "000"}`,
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <header className="flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6 shadow-sm">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center">
                ...
                />
                <span className="ml-2 hidden text-sm font-medium text-[#0F172A] sm:block">Admin</span>
          ...
          <main className="flex-1 overflow-y-auto bg-[#F8FAFC] p-6 lg:p-8">
            ...
            <div className="mb-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {stats.map(({ title, value, icon: Icon, change, changeTone, iconBg, iconColor }) => (
                <div key={title} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-[#475569]">{title}</p>
                      <h3 className="mt-1 text-2xl font-bold text-[#0F172A]">{loading ? "Loading..." : value}</h3>
                    </div>
                  ...
                  <div className="mt-4 flex items-center text-sm">
                    <span className={`font-medium ${changeTone} flex items-center`}>
            ...
            <div className="mb-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
                ...
                <Link href="#" className="text-sm font-medium text-[#4F46E5] transition-colors hover:text-[#4338CA]">
                  View All
                </Link>
              </div>
                    ...
                    {recentOrders.length > 0 ? (
                      recentOrders.map((order) => (
                        <tr key={order.id} className="transition-colors hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 text-[#94A3B8]">{order.date}</td>
                          <td className="px-6 py-4">
                            <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${order.statusClass}`}>
                              {order.status}
                            </span>
                          </td>
